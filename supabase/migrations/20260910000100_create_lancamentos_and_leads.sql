-- Catálogo de lançamentos imobiliários + captação de leads do site
-- Projeto: site-corretor
...
-- Visitantes do site só podem consultar lançamentos explicitamente publicados.
...
-- O formulário pode criar um lead, mas visitantes não podem listar, ler, alterar ou apagar leads.
drop policy if exists "Site pode criar leads" on public.leads_do_site;
create policy "Site pode criar leads"
on public.leads_do_site
for insert
to anon, authenticated
with check (fonte = 'site');

-- Usuários autenticados podem administrar o catálogo e os leads no painel seguro.
...
comment on table public.lancamentos is 'Catálogo de empreendimentos sincronizado com o Notion e publicado no site conforme visibilidade_site.';
comment on table public.leads_do_site is 'Leads enviados pelo formulário público do site. Leitura pública bloqueada por RLS.';
comment on column public.lancamentos.notion_page_id is 'ID da página de origem no Notion para futura sincronização.';
comment on column public.lancamentos.visibilidade_site is 'Controla se o lançamento pode aparecer no catálogo público.';

-- Conferência rápida após aplicar:
-- select column_name, data_type from information_schema.columns
-- where table_schema = 'public' and table_name in ('lancamentos', 'leads_do_site')
-- order by table_name, ordinal_position;
-- select count(*) from public.lancamentos;
-- select count(*) from public.leads_do_site;

-- Observação de segurança:
-- Nunca coloque a service_role key no HTML, no GitHub ou na Vercel como variável pública.
-- O site deve usar somente a chave publicável/anon e as políticas RLS acima.
